...
const uploadDesign = asyncHandler(async (req, res) => {
  ...
  const getDesign = asyncHandler(async (req, res) => {
    try {
      ...
      const { id } = req.params; // Assuming the tailor ID is provided in the route parameter
      ...
      const querySnapshot = await designCollection.where('tailor_id', '==', id).get();
  ...
  const updateDesign = asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (!designDoc.exists) {
        ...
        if (designprice) {
          updateFields.designprice = designprice;
        }
        if (days) {
        ...
        await designRef.update(updateFields);
